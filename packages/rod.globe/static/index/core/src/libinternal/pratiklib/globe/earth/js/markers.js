...
function attachMarkerToCountry( countryName, importance ){
	...
	// else
	{
		//rod prepare lien vers taxo avec appel ajax vers drupal (OU APRES TESTER countryData avec ajout dans geopins.js lien de la taxo)
		var taxolink=configFromRKR['pathtoroot'];
		if(countryAll[countryName]['tid'] != null && countryAll[countryName]['tid'] != 'undefined' && countryAll[countryName]['tid'] != '')
		{
			var taxoprelink='/'+configFromRKR['rootsubfolder']+'/index.php?page=globe&zonegeo=';
			if(configFromRKR['linkzonetorkr'] != null && configFromRKR['linkzonetorkr'] != 'undefined' && configFromRKR['linkzonetorkr'] != '')
				taxoprelink='/'+configFromRKR['rootsubfolder']+configFromRKR['linkzonetorkr'];
			taxoprelink=taxoprelink.replace("//","/");
			taxoprelink=taxoprelink.replace("//","/");
			taxolink=taxoprelink+countryAll[countryName]['tid'];
		}
		nameLayer.innerHTML = '<a target="_parent" href="'+taxolink+'">'+countryName.replace(' ','&nbsp;')+'</a>';
	}
