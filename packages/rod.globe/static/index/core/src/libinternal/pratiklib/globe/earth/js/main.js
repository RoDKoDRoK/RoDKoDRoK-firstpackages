...
//configFromRKR
var configFromRKR = Array();

configFromRKR['rootsubfolder']='';
var basePath=parent.document.getElementById('basepath'); //recup delta block
if(basePath != null && basePath != 'undefined' && basePath != '')
	configFromRKR['rootsubfolder'] = basePath.value;
/* */
configFromRKR['defaultcountry']='FRANCE';
configFromRKR['pathtoroot']='http://localhost/'+configFromRKR['rootsubfolder']+'/';
configFromRKR['pathtoearth']='http://localhost/'+configFromRKR['rootsubfolder']+'/core/src/libinternal/pratiklib/globe/earth/';
configFromRKR['linkzonetorkr']='/index.php?page=globe&zonegeo=';
configFromRKR['preloadoldcountryfile']='1';
configFromRKR['loadnewcountryfile']='0';
configFromRKR['dbtabletouse']='zonegeo';
configFromRKR['pathtogreymap']='http://localhost/'+configFromRKR['rootsubfolder']+'/core/src/libinternal/pratiklib/globe/earth/'+'images/map_indexed.png';
/* */
...
var old  = configFromRKR['pathtoearth']+'country_iso3166.json'; //old isofile, set oldIsoFilePreload to true to use it
var oldIsoFilePreload = configFromRKR['preloadoldcountryfile'];
var newIsoFileLoad = configFromRKR['loadnewcountryfile'];
var isoFile = configFromRKR['pathtoroot']+'index.php?chain=json&ws='+configFromRKR['dbtabletouse'];
var oldLatlonFile = configFromRKR['pathtoearth']+'country_lat_lon.json';
/*
//var loaded in start()
var oldIsoFile = configFromRKR['pathtoearth']+'country_iso3166.json'; //old isofile, set oldIsoFilePreload to true to use it
var oldIsoFilePreload = configFromRKR['preloadoldcountryfile'];
var newIsoFileLoad = configFromRKR['loadnewcountryfile'];
var isoFile = configFromRKR['pathtoroot']+'taxo-zone-geo-json/'+configFromRKR['taxotouse'];
var oldLatlonFile = configFromRKR['pathtoearth']+'country_lat_lon.json';
*/
...
//all country data from taxonomy drupal
var countryAll = new Array();	

//country color map avec niveaux de gris chargés depuis taxonomy drupal
var countryColorMap = new Array();
//	ordered lookup list for country color index
//	used for GLSL to find which country needs to be highlighted


//taille sphere
var tailleSphere = 100;
...
function start( e ){	
	...
	else
	{
		...
		{
			//init var globale
			//configFromRKR['pathtoroot']+='/'+configFromRKR['rootsubfolder']+'/';
			oldIsoFile = configFromRKR['pathtoearth']+'country_iso3166.json';
			oldIsoFilePreload = configFromRKR['preloadoldcountryfile'];
			newIsoFileLoad = configFromRKR['loadnewcountryfile'];
			isoFile = configFromRKR['pathtoroot']+'index.php?chain=json&ws='+configFromRKR['dbtabletouse'];
			oldLatlonFile = configFromRKR['pathtoearth']+'country_lat_lon.json';
			
			if(oldIsoFilePreload)
			{
				countryColorMap = {'PE':1,
			...
			mapIndexedImage.src = configFromRKR['pathtogreymap']; //+'images/map_indexed.png';
			mapIndexedImage.onload = function()
			{
				mapOutlineImage = new Image();
				mapOutlineImage.src = configFromRKR['pathtoearth']+'images/mainmap.png'; // 'images/map_outline.png';
...
function initScene() {
	...
	//add bumpmap ti uniforms
	//uniforms[ "enableBump" ].value = true;
	//uniforms[ "bumpMap" ].value = THREE.ImageUtils.loadTexture( 'images/rev_elev.png' );
...
// 			specular: 	0x333333,
	...
	sphere = new THREE.Mesh( new THREE.SphereGeometry( tailleSphere, 40, 40 ), shaderMaterial );				
	...
	//rod reload selectableCountries from countryAll
	selectableCountries=new Array();
	for( var tmp in countryAll )
		selectableCountries.push( tmp );
	//...
...
*/
	...
	//selectVisualization( timeBins, '2010', ['FRANCE'], ['Military Weapons','Civilian Weapons', 'Ammunition'], ['Military Weapons','Civilian Weapons', 'Ammunition'] );					
	selectVisualization( timeBins, '2010', [configFromRKR['defaultcountry']], ['Military Weapons','Civilian Weapons', 'Ammunition'], ['Military Weapons','Civilian Weapons', 'Ammunition'] );
...
function highlightCountry( countries ){	
	...
	for( var i in countryCodes ){
		...
		var mapColor = '';
		if(countryData[countries[i]])
			mapColor = countryData[countries[i]].mapColor;
		// var fillCSS = '#ff0000';
		var fillCSS = '#333333';
